...
function groupPhotosByInspectionId(photos: InspectionPhoto[]): Map<string, InspectionPhoto[]> {
  const m = new Map<string, InspectionPhoto[]>();
  for (const p of photos) {
    const list = m.get(p.inspection_id) ?? [];
    list.push(p);
    m.set(p.inspection_id, list);
  }
  return m;
}
...
export default async function AdminInspectionsPage() {
  ...
  const allPhotos = await listPhotosForInspectionIds(inspectionIds);
  const photosByInspection = groupPhotosByInspectionId(allPhotos);
    ...
    <div>
      ...
      {inspections.length === 0 ? (
        ...
        <div className="space-y-6">
          {inspections.map((i) => {
            const photos = photosByInspection.get(i.id) ?? [];
